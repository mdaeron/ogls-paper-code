def YorkReg(X, Y, sX, sY,
	rXY=0.,
	max_iter = 1e3,
	cvg_limit=1e-15,
	rel_cvg = True,
	verbose = False,
	overdispersion_scaling = False,
	underdispersion_scaling = False,
	):
	'''
	...
	overdispersion_scaling : boolean
		If True, and reduced chi-square is greater than 1, multiply the covariance matrix
		of best-fit parameters by the reduced chi-square.

	underdispersion_scaling : boolean
		If True, and reduced chi-square is less than 1, multiply the covariance matrix
		of best-fit parameters by the reduced chi-square.
	'''
	...
	CM = np.array([[sB2,cAB],[cAB,sA2]])

	chi2 = sum(W * (Y - B * X - A) ** 2)
	Nf = X.size - 2
	if Nf > 0:
		if overdispersion_scaling and chi2/Nf > 1:
			CM *= chi2/Nf
		if underdispersion_scaling and chi2/Nf < 1:
			CM *= chi2/Nf
